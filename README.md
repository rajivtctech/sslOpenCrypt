...
Grab the binary for your platform from the [latest release](https://github.com/rajivtctech/sslOpenCrypt/releases/latest). All three are self-contained and run straight from a USB pendrive — there is no installer.

| Platform | File | OpenSSL |
|---|---|---|
...
```
sslOpenCrypt-Windows.exe --cli --mode version --pretty
```

GnuPG is **not** bundled on any platform. Without it the GPG panel is disabled and everything else works normally; install [Gpg4win](https://gpg4win.org) (Windows) or `gnupg` (Linux/macOS) to enable it. India DSC token signing additionally needs your token vendor's PKCS#11 driver.

> The bundled OpenSSL is © The OpenSSL Project under Apache-2.0, invoked as a separate process; its licence text ships alongside it. sslOpenCrypt itself is GPL v3.
